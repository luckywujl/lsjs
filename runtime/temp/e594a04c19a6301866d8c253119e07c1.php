...
<script type="text/javascript">
    ...
    <div class="form-group">
        ...
        <div class="col-xs-12 col-sm-8">
            <input id="c-company_regtime" class="form-control datetimepicker" data-date-format="YYYY-MM-DD HH:mm:ss" data-use-current="true" name="row[company_regtime]" type="text" value="<?php echo $row['company_regtime']?datetime($row['company_regtime']):''; ?>">
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-xs-12 col-sm-8">
            ...
            <label for="row[company_status]-<?php echo $key; ?>"><input id="row[company_status]-<?php echo $key; ?>" name="row[company_status]" type="radio" value="<?php echo $key; ?>" <?php if(in_array(($key), is_array($row['company_status'])?$row['company_status']:explode(',',$row['company_status']))): ?>checked<?php endif; ?> /> <?php echo $vo; ?></label> 
        ...
        <div class="col-xs-12 col-sm-8">
            ...
            <select  id="c-company_type" class="form-control selectpicker" name="row[company_type]">
                <?php if(is_array($companyTypeList) || $companyTypeList instanceof \think\Collection || $companyTypeList instanceof \think\Paginator): if( count($companyTypeList)==0 ) : echo "" ;else: foreach($companyTypeList as $key=>$vo): ?>
                    <option value="<?php echo $key; ?>" <?php if(in_array(($key), is_array($row['company_type'])?$row['company_type']:explode(',',$row['company_type']))): ?>selected<?php endif; ?>><?php echo $vo; ?></option>
                <?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
    ...
    <div class="form-group">
        ...
        <div class="col-xs-12 col-sm-8">
            <input id="c-company_asofdate" class="form-control" name="row[company_asofdate]" type="text" value="<?php echo htmlentities($row['company_asofdate']); ?>">
        </div>
    </div>
