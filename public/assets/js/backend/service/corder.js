            ...
            Table.api.bindevent(table);
	         ...
	         //添加明细
       		$(document).on("click",".btn-newadd",function () {
       		  if ($("#c-order_user_id").val()!=='') {
       			...
       			Fast.api.open('service/corder/detailadd?user_id='+$("#c-order_user_id").val(),'添加产品',{//?card_code=" + $(this).attr("id") + "&multiple=" + multiple + "&mimetype=" + mimetype, __('Choose'), {
	            area:['85%', '95%'],
		           callback: function (data) {	
	         ...
	         //保存
       		$(document).on("click",".btn-verify",function () {
       		  if ($("#c-order_id").val()=='') {
       			$("#add-form").attr("action","service/corder/add").submit(); 
       		} else{
