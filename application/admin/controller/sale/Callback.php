...
class Callback extends Backend
{
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $list = $this->model
            	 ->field('log_code,log_type,log_date,log_saleman,log_user_name,log_user_contact,log_tel,log_address,log_operator,log_status,count(*) as tasknumber')
                ->where($where)
                ->where(['log_saleman'=>$this->auth->nickname,'log_status'=>['in',[2,3,4,5]]])  //只求是我销售的和未结单的
                ->group('log_code,log_type,log_date,log_saleman,log_user_name,log_user_contact,log_tel,log_address,log_operator,log_status')
                ->order($sort, $order)
                ->paginate($limit);
    ...
    public function edit($ids = null)
    {
        ...
        $list = $this->model
            	 ->field('log_code,log_type,log_date,log_saleman,log_user_name,log_user_contact,log_tel,log_address,log_operator,log_status,company_id,count(*) as tasknumber')
                ->where(['log_saleman'=>$this->auth->nickname,'log_code'=>$ids,'log_status'=>$log_status,'company_id'=>$this->auth->company_id])  //只求我的和未结单的
                ->group('log_code,log_type,log_date,log_saleman,log_user_name,log_user_contact,log_tel,log_address,log_operator,log_status,company_id')
                ->select();
        $row = $list[0];
