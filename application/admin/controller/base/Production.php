...
class Production extends Backend
{
    ...
    /**
     * Production模型对象
     * @var \app\admin\model\base\Production
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\base\Production;
    ...
    public function del($ids = "")
    {
        ...
        if ($ids) {
            ...
            try {
                foreach ($list as $k => $v) {
                	 if($v['production_stock_number']==0) {
                    $count += $v->delete();
                    }
                }
                Db::commit();
            ...
            if ($count) {
                $this->success(__('已为您删除选中的且库存为0的产品，库存不为0的产品无法删除！'));
            } else {
    ...
    public function getproducttype()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $list = $this->model
                ->where($where)
                ->where('production_name',$product['production_name'])
                ->order($sort, $order)
                ->paginate($limit);
            $result = array("total" => $list->total(), "rows" => $list->items());
            return json($result);
