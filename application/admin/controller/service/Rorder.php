...
class Rorder extends Backend
{
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\service\Rorder;
    ...
    public function add()
    {
    	  ...
    	  $detailtemp = new service\Detailtemp();
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                $detailtemp_info = $detailtemp
                		->where(['detail_main_id'=>2,'detail_operator'=>$this->auth->nickname,'company_id'=>$this->auth->company_id])
                		->order('detail_id asc')
                		->select();
    ...
    public function detailindex()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $list = $detailtemp
                ->where($where)
                ->where(['detail_main_id'=>2,'detail_operator'=>$this->auth->nickname])  //2为售后销售
                ->order($sort, $order)
                ->paginate($limit);
    ...
    public function detailadd()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                $sn = $detailtemp
                		->where(['detail_main_id'=>2,'company_id'=>$this->auth->company_id,'detail_operator'=>$this->auth->nickname])
                		->count();
                $params['detail_sn'] = $sn+1;
                $params['detail_main_id'] = 2;
    ...
    */
    ...
    public function cleardetail()
    {
        ...
        $detai_result = $detailtemp
           	   ->where($where)
            	->where(['detail_main_id'=>2,'detail_operator'=>$this->auth->nickname])
            	->delete();
          $this->success('OK',null,null);
